...
async function createTables(env: Env): Promise<void> {
  ...
  try { await runSQL(env, "ALTER TABLE knowledge_entries ADD COLUMN author TEXT DEFAULT ''"); } catch (e) {}
  try { await runSQL(env, "ALTER TABLE knowledge_entries ADD COLUMN is_original INTEGER DEFAULT 0"); } catch (e) {}
  try { await runSQL(env, "ALTER TABLE knowledge_entries ADD COLUMN published_at TEXT DEFAULT ''"); } catch (e) {}
